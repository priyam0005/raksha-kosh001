...
// ── NEW: allowlist of legitimate PNG tEXt / iTXt keyword prefixes ──
// Keys NOT in this set are treated as suspicious unknown fields.
const PNG_TEXT_ALLOWED_KEYS = new Set([
  ...
  // libpng / ImageMagick standard keys
  "date:create",
  "date:modify",
  "date:timestamp",
]);

// ── NEW: Maximum allowed length for any single metadata value ──
...
function readRational(buf, off, le) {
  const num = safeUInt32(buf, off, le);
  const den = safeUInt32(buf, off + 4, le);
  if (num === null || den === null || den === 0) return null;
  return num / den;
}
function readSignedRational(buf, off, le) {
  const num = safeInt32(buf, off, le);
  const den = safeInt32(buf, off + 4, le);
  if (num === null || den === null || den === 0) return null;
  return num / den;
}
...
function readIFDValue(buf, type, count, valueOffset, le) {
  ...
  switch (type) {
    case 1:
    case 7:
      if (count === 1) return buf[dataOffset];
      return Array.from(safeSlice(buf, dataOffset, dataOffset + count) || []);
    case 2: {
      const raw = safeSlice(buf, dataOffset, dataOffset + count);
      if (!raw) return undefined;
      return raw.toString("latin1").replace(/\0.*$/, "").trim();
    }
    case 3:
      if (count === 1) return safeUInt16(buf, dataOffset, le);
      return Array.from({ length: count }, (_, i) =>
        safeUInt16(buf, dataOffset + i * 2, le),
      );
    case 4:
      if (count === 1) return safeUInt32(buf, dataOffset, le);
      return Array.from({ length: count }, (_, i) =>
        safeUInt32(buf, dataOffset + i * 4, le),
      );
    case 5:
      if (count === 1) return readRational(buf, dataOffset, le);
      return Array.from({ length: count }, (_, i) =>
        readRational(buf, dataOffset + i * 8, le),
      );
    case 9:
      if (count === 1) return safeInt32(buf, dataOffset, le);
      return Array.from({ length: count }, (_, i) =>
        safeInt32(buf, dataOffset + i * 4, le),
      );
    case 10:
      if (count === 1) return readSignedRational(buf, dataOffset, le);
      return Array.from({ length: count }, (_, i) =>
        readSignedRational(buf, dataOffset + i * 8, le),
      );
    default:
      return undefined;
  }
...
function parseIFD(buf, ifdOffset, le, tagMap, result, depth, errors) {
  if (depth > MAX_IFD_DEPTH) {
    errors.add("Max IFD depth exceeded");
    return;
  }
  if (ifdOffset <= 0 || ifdOffset + 2 > buf.length) return;

  const entryCount = safeUInt16(buf, ifdOffset, le);
  if (entryCount === null || entryCount > MAX_IFD_ENTRIES) {
    errors.add(`Abnormal IFD entry count: ${entryCount}`);
    return;
  }
  ...
  for (let i = 0; i < entryCount; i++) {
    ...
    if (tagId === 0x8769 || tagId === 0xa005) {
      const subPtr = safeUInt32(buf, entryOff + 8, le);
      if (subPtr)
        parseIFD(buf, subPtr, le, EXIF_TAGS, result, depth + 1, errors);
      continue;
    }
    if (tagId === 0x8825) {
      const gpsPtr = safeUInt32(buf, entryOff + 8, le);
      if (gpsPtr)
        parseIFD(buf, gpsPtr, le, GPS_TAGS, result, depth + 1, errors);
      continue;
    }
  ...
  if (nextOff && nextOff !== ifdOffset) {
    parseIFD(buf, nextOff, le, tagMap, result, depth + 1, errors);
  }
}

function parseTIFFBlock(buf, errors) {
  if (buf.length < 8) {
    errors.add("TIFF block too small");
    return {};
  }
  const byteOrder = buf.slice(0, 2).toString("ascii");
  if (byteOrder !== "II" && byteOrder !== "MM") {
    errors.add("Invalid TIFF byte-order marker");
    return {};
  }
  const le = byteOrder === "II";
  if (safeUInt16(buf, 2, le) !== 42) {
    errors.add("Invalid TIFF magic number");
    return {};
  }
  const ifd0Offset = safeUInt32(buf, 4, le);
  if (!ifd0Offset) {
    errors.add("Cannot read IFD0 offset");
    return {};
  }
  const result = {};
...
function parseJPEG(buf, errors) {
  ...
  while (offset + 4 <= buf.length) {
    if (buf[offset] !== 0xff) {
      errors.add(`Lost sync at offset ${offset}`);
      break;
    }
    ...
    if (segLen === null || segLen < 2) {
      errors.add("Invalid JPEG segment length");
      break;
    }
    const segStart = offset + 4;
    const segEnd = offset + 2 + segLen;
    if (segEnd > buf.length) {
      errors.add("JPEG segment extends beyond file");
      break;
    }
    ...
    if (markerByte === 0xe1) {
      ...
      if (header === "Exif\0\0")
        Object.assign(meta, parseTIFFBlock(payload.slice(6), errors));
    }
    offset = segEnd;
  }
  return meta;
...
function parsePNG(buf, errors, unknownKeys) {
  ...
  let iendOffset = null; // ← track where IEND ends
  ...
  while (offset + 8 <= buf.length) {
    ...
    if (dataEnd > buf.length) {
      errors.add("PNG chunk extends beyond file");
      break;
    }
    ...
    if (chunkType === "tEXt") {
      ...
      Object.assign(
        meta,
        parseTIFFBlock(buf.slice(dataStart, dataEnd), errors),
      );
    } else if (chunkType === "IEND") {
      // IEND data must be empty (chunkLen === 0); flag if it isn't
      if (chunkLen !== 0)
        errors.add("IEND chunk has non-zero length — file may be crafted");
      iendOffset = dataEnd + 4; // byte right after IEND CRC
      break;
    }
  ...
  // A valid PNG must end exactly at IEND+CRC. Anything after is
  // a steganographic hiding place or an injected payload.
  if (iendOffset === null) {
    ...
    const trailingSample = buf.slice(
      iendOffset,
      Math.min(iendOffset + 64, buf.length),
    );
    // Treat ANY trailing data as a threat — even if it looks innocent,
    // it has no legitimate reason to exist in a valid PNG.
    errors.add(
      `${trailingLen} byte(s) of data found after IEND chunk — possible injected payload`,
    );
...
function parseWebP(buf, errors) {
  ...
  while (offset + 8 <= buf.length) {
    ...
    const dataEnd = dataStart + chunkLen;
    if (dataEnd > buf.length) {
      errors.add("WebP chunk extends beyond file");
      break;
    }
    if (chunkId === "EXIF")
      Object.assign(
...
function parseExifTimestamp(str) {
  ...
  // ISO 8601 format used in PNG tEXt: "2024-06-22T09:42:53+00:00"
  const ts = Date.parse(str);
  return Number.isFinite(ts) ? ts : null;
}

/* ─────────────────── NEW: VALUE SAFETY CHECK ────────── */

/**
 * Returns true if the string value looks suspicious beyond the existing regex patterns:
 * - Exceeds maximum allowed length
 * - Contains high ratio of non-ASCII / binary bytes
 */
function valueLooksSuspicious(str) {
  if (str.length > MAX_VALUE_LENGTH)
    return { suspicious: true, reason: "value-too-long" };
  ...
  if (sampleLen > 0 && nonPrintable / sampleLen > 0.1) {
    return { suspicious: true, reason: "binary-or-control-chars" };
  }
...
function layer4MetadataForensics(fileBuffer) {
  ...
  if (!Buffer.isBuffer(fileBuffer)) {
    report.errors.push("Input must be a Buffer");
    report.safe = false;
    return report;
  }
  if (fileBuffer.length < 12) {
    report.errors.push("File too small");
    report.safe = false;
    return report;
  }
  if (fileBuffer.length > MAX_FILE_SIZE) {
    report.errors.push(`File size exceeds maximum`);
    report.safe = false;
    return report;
  }

  report.fileType = detectFileType(fileBuffer);
  if (report.fileType === "unknown")
    report.warnings.push("Unrecognised file type; metadata extraction skipped");

  const parseErrors = new Set();
  // ── FIX 2: pass unknownKeys collector into parsePNG ──
  ...
  try {
    switch (report.fileType) {
      case "jpeg":
        all = parseJPEG(fileBuffer, parseErrors);
        break;
      case "png":
        all = parsePNG(fileBuffer, parseErrors, unknownPNGKeys);
        break;
      case "webp":
        all = parseWebP(fileBuffer, parseErrors);
        break;
      case "tiff":
        all = parseTIFFBlock(fileBuffer, parseErrors);
        break;
    }
  } catch (err) {
    parseErrors.add(`Unexpected parse error: ${err.message}`);
  }
  ...
  for (const e of parseErrors) {
    ...
    if (
      ...
      e.includes("IEND") ||
      e.includes("crafted") ||
      e.includes("beyond file")
    ) {
      report.safe = false;
    }
  }

  // unknown PNG tEXt keys are an instant fail
  if (unknownPNGKeys.size > 0) {
    const keyList = [...unknownPNGKeys].join(", ");
    report.injectionThreats.push({
      field: "__png_text_keys__",
      value: keyList,
      reason: "unknown-png-text-key",
    });
    report.warnings.push(
      `Unknown/non-standard PNG tEXt keys detected: ${keyList}`,
    );
    report.safe = false;
  }
  ...
  if (Object.keys(all).length > MAX_METADATA_FIELDS) {
    report.errors.push(
      "Excessive metadata fields detected; possible crafted file",
    );
    report.safe = false;
  ...
  for (const [field, value] of Object.entries(all)) {
    ...
    // ── FIX 4: check every value for binary/suspicious content ──
    ...
    if (suspicious) {
      ...
      report.safe = false;
      continue; // no need to run regex patterns on already-flagged field
    }

    // Existing injection pattern scan
  ...
  //timestamp check now also covers PNG date:* keys
  // ── Improved timestamp validation ──
  const nowUTC = Date.now();
  const FUTURE_TOLERANCE_MS = 5 * 60 * 1000; // allow 5 minutes clock skew

  for (const f of DATETIME_FIELDS) {
    if (!all[f]) continue;

    const rawValue = String(all[f]).trim();
    const ts = parseExifTimestamp(rawValue);

    if (ts === null) continue; // ignore unparsable timestamps safely

    // Only flag if clearly beyond tolerance
    if (ts - nowUTC > FUTURE_TOLERANCE_MS) {
      report.warnings.push(
        `Timestamp in "${f}" appears far in the future: ${rawValue}`,
      );
      report.timestampSuspicious = true;
      report.safe = false;
    }
  }
  ...
  for (const [key, value] of Object.entries(all)) {
    if (!stripSet.has(key) && !threatFieldSet.has(key)) {
      report.cleanMetadata[key] = value;
    }
  }
