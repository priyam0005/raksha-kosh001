...
// ── FIX 1: 26-hour grace buffer — covers any UTC offset + minor clock skew ──
const TIMESTAMP_GRACE_MS = 26 * 60 * 60 * 1000;
...
const DATETIME_FIELDS = [
  ...
  "date:create", "date:modify", "date:timestamp", "Creation Time",
];
...
function layer4MetadataForensics(fileBuffer) {
  ...
  // ── FIX 1 applied: compare against now + 26hr grace buffer ──
  const now = Date.now();
  const threshold = now + TIMESTAMP_GRACE_MS;
  for (const f of DATETIME_FIELDS) {
    if (!all[f]) continue;
    const ts = parseExifTimestamp(String(all[f]));
    if (ts !== null && ts > threshold) {
      report.warnings.push(`Timestamp in "${f}" is in the future: ${all[f]}`);
